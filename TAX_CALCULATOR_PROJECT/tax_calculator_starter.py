...
def calculate_income_tax(annual_income):
    ...
    tax = 0.0
    remaining = annual_income - 1800000
    ...
    for size, rate in brackets:
        ...
        taxable_amount = min(remaining, size)
...
def main():
    ...
    taxes = list(map(calculate_income_tax, incomes))  # YOUR CODE HERE - replace with map()
    ...
    high_earners = lis(filter(lambda x:x >= 430000, incomes)) # YOUR CODE HERE - replace with filter()
    ...
    sorted_incomes_taxes = []  # YOUR CODE HERE - replace with zip() and sorted()
